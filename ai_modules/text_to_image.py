...
class Text_to_image:
    ...
    def character_image(self, title, eng_title, user_id):
        ...
        try:
            ...
            initial_image = Image.new("RGB", (1920, 1024), (0, 0, 0))

        answers = self.stability_api.generate(
            prompt=[
                generation.Prompt(
                    text=f"{eng_title} ,ghibli style, bangs, long sleeves, no glasses,necklace, parted lips, smile, solo, alone, one person, white background, looking at viewer, ((masterpiece)) <lora:ghibli_style:0.65>",
    ...
    def title_image(self, eng_title: str, title: str, user_id, seed, initial_image: Image.Image) -> (Image.Image, int, str):
        ...
        answers = self.stability_api.generate(
            prompt=[
                    generation.Prompt(text=f"{initial_image} is main character of fairytale", parameters=generation.PromptParameters(weight=2.0)),
                    generation.Prompt(text=f"{eng_title}, Please use the main character to make a cover of the fairytale, No protagonist changes",parameters=generation.PromptParameters(weight=1.9)),
    ...
    def story_image(self, no_title_ko_pmt: str, no_title_eng_pmt: str, title: str, user_id, page: int, initial_seed, initial_image: Image.Image) -> str:
        ...
        answers = self.stability_api.generate(
            prompt=[
                    generation.Prompt(text=f"{initial_image} is main character of fairytale", parameters=generation.PromptParameters(weight=2.0)),
                    generation.Prompt(text=f"the same type of painting as that of the {initial_image}, Create an image according to the story : {no_title_eng_pmt}", parameters=generation.PromptParameters(weight=1.8)),
                    generation.Prompt(text=f" No protagonist changes",parameters=generation.PromptParameters(weight=1.9)), # 긍정 프롬프트 추가
            ...
            height=1024,
            samples=1,
            guidance_prompt="""8k, best quality, masterpiece, ultra detail, cute face, smile, beautiful detailed eyes, pretty, smooth lighting, cinematic lighting, close-up""" ,
            guidance_strength=0.4,
            guidance_models=["clip_vit_base_patch16_384", "clip_vit_base_patch16_384"]
        )
    ...
    def add_text_to_image(self, img: Image.Image, text: str, position: tuple = (10, 10), font_size: int = 25):
        ...
        for line in lines:
            ...
            draw.text((position[0], y_position), line, font=font, fill=(255, 255, 255))
...
class T2I_generater_from_prompts:
    ...
    def title_images_from_prompt(self, eng_title, title, user_id, seed, initial_image:Image.Image):
        ...
        img_path = t2i_gen.title_image(eng_title, title, user_id=user_id, seed=seed, initial_image=initial_image)
        return img_path

    def story_images_from_prompts(self, no_title_ko_pmt, no_title_eng_pmt, title, user_id, initial_seed, initial_image:Image.Image):
