...
class TextToImage:
    ...
    # 이미지 생성 함수
    def generate_image_with_stability_ai(self, english_prompts : str, korean_prompts, image_folder, STABILITY_KEY, font_path):
...
def generate_video_with_images_and_text(english_prompts, korean_prompts, audio_folder, image_folder,
    ...
    try:
        # 이미지 생성
        t2i = TextToImage(english_prompts, korean_prompts, audio_folder, image_folder, output_video_path, font_path, STABILITY_KEY)
        image_paths = t2i.generate_image_with_stability_ai(english_prompts, korean_prompts, image_folder, STABILITY_KEY, font_path)
        # 오디오 파일 경로
        ...
        video.write_videofile(output_video_path, codec="libx264", audio_codec="aac", temp_audiofile="temp-audio.m4a", remove_temp=True, verbose=False)
